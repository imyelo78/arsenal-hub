...
const competition = ref<'pl' | 'cl'>('pl')

const { data, pending } = await useFetch(() => `/api/standings?comp=${competition.value}`)
...
<template>
  <div class="page-enter">
    <!-- Page header -->
    <div class="mb-8">
      <h1 class="text-2xl sm:text-3xl font-bold text-arsenal-ink tracking-tight">
        {{ competition === 'pl' ? t('standings.title') : t('standings.clTitle') }}
      </h1>
      <p class="text-arsenal-muted mt-1.5 text-sm">
        {{ competition === 'pl' ? t('standings.subtitle') : t('standings.clSubtitle') }}
      </p>
    </div>

    <!-- Competition tabs -->
    <div class="flex bg-arsenal-line2 rounded-lg p-0.5 w-fit mb-6">
      <button
        v-for="comp in (['pl', 'cl'] as const)"
        :key="comp"
        @click="competition = comp"
        class="px-4 py-1.5 text-sm font-medium rounded-md transition-all"
        :class="competition === comp
          ...
          >
            ...
            <td class="text-center py-3 px-2">
              <span
                ...
                :class="{
                  'text-arsenal-muted': competition === 'cl'
                }"
              >
                {{ row.rank }}
              </span>
    ...
    <!-- Legend (英超) -->
    <div v-if="competition === 'pl'" class="flex flex-wrap gap-4 mt-4 text-xs text-arsenal-muted">
